    ...
    <div className="main-container">
      <Container>
        <Row>
          ...
          <Col xs={12} md={8}>
            {tweets.map(({ id, text, quoted_status, created_at, entities }, i) => {
              return (
                <Row key={`row-${id}`} id={id}>
                  <Card text="dark" className="card-tweet">
                    <Card.Img variant="top" src={props.profile_image_url} className="card-tweet__img" />
                    <Card.Body className="card-tweet__body">
                      <Card.Title className="card-tweet__title">
                        <a href={`#${id}`}>
                          @{props.screen_name}
                        </a>
                        <small className="card-tweet__time">{getDiffNow("2018-05-27T20:23:00")}</small>
                      </Card.Title>
                      <Card.Text>
                        {getRichTweet(text, entities)}
                        {
                          quoted_status &&
                          <Alert as="span" variant="secondary" className="card-reetweet">
                            <b> {quoted_status.user.name} <a>@{quoted_status.user.screen_name}</a></b>
                            <hr style={{ margin: ".5rem 0" }} />
                            {getRichTweet(quoted_status.text, quoted_status.entities)}
                          </Alert>
                        }
                      </Card.Text>
                    </Card.Body>
...
function getRichTweet(text = "", entities) {
  const components = [];
  let i = 0;
  let value = "";
  let urls = [];
  entities.hashtags.forEach(({ indices }) => {
    ...
    components.push(
      <a href="#">{text.substring(indices[0], indices[1])}</a>
    );
    i = indices[1];
  })
